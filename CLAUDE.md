...
- **매니페스트 추적 대상이 아니다.** 1회성 설치이며, 충돌·삭제 추적을 하지 않는다. 최신 버전이 필요하면 사용자가 다시 설치를 요청할 때 소스에서 새로 받아 덮어쓴다.
...
- 권한 문자열에 `{{홈}}` 플레이스홀더가 포함된 경우, 설치 시점에 실제 홈 디렉토리 절대 경로로 치환한다 (예: `/Users/garshion`, `/home/user`).
- 치환 후의 문자열을 `settings.json` 에 기록하고, 매니페스트의 `entries` 에도 치환된 값을 저장한다.
- 이하 "병합 동작" 및 "설치/갱신 의사결정" 에서 항목 `E` 는 치환 완료된 값을 가리킨다.
